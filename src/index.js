...
const { flattenColors } = require('./colors/themes/utils/index')
...
const mainFunction = ({ addBase, addComponents, addUtilities, config }) => {
        ...
        config('daisyui.themes').forEach((item, index) => {
            if (typeof item === 'object' && item !== null) {
                Object.entries(item).forEach(([customThemeName, customThemevalue]) => {
                    customThemevalue = flattenColors(customThemevalue)
                    existingTheme = themes.hasOwnProperty(`theme-${customThemeName}`) ? themes[`theme-${customThemeName}`] : {}
                    customThemevalue = { ...existingTheme, ...customThemevalue }
                    includedThemesObj['theme-' + customThemeName] = convertThemeColorsToHsl(customThemevalue)
                })
            }
        })
    }
    ...
    if (Array.isArray(config('daisyui.themes'))) {
        config('daisyui.themes').forEach((theme, index) => {
            ...
            } else if (includedThemesObj.hasOwnProperty('theme-' + theme)) {
    ...
    themeOrder.forEach((themeName, index) => {
        if (index === 0) { // first theme as root
            addBase({ [':root']: includedThemesObj['theme-' + themeName] })
        } else if (index === 1) {
            // auto dark
            if (themeOrder[0] != 'dark' && themeOrder.includes('dark')) {
                addBase({ ['@media (prefers-color-scheme: dark)']: { [':root']: includedThemesObj['theme-dark]'] } })
            }
            // theme 0 with name
            addBase({ ['.theme-' + themeOrder[0]]: includedThemesObj['theme-' + themeOrder[0]] })
            // theme 1 with name
            addBase({ ['.theme-' + themeOrder[1]]: includedThemesObj['theme-' + themeOrder[1]] })
        } else {
            addBase({ ['.theme-' + themeName]: includedThemesObj['theme-' + themeName] })
        }
    })
